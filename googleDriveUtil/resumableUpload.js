...
function resumableUpload() {
	...
	this.progressReportInterval = 5000;
};
	...
	var health = setInterval(function () {
		self.getProgress(function (err, res, body) {
			if (!err && typeof res.headers.range !== 'undefined') {
				var fileSize = fs.statSync(self.filepath).size;
				var percent = parseInt(res.headers.range.substring(8)) / fileSize;
				self.emit('progress', percent);
			}
		});
	}, self.progressReportInterval);
